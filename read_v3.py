...
class QM_reader(object):
    ...
    def read_obs(self, folder):
        ...
        pvdz_dlpno = self.read_ccsdt(path + 'pvdz_DLPNO_CCSDt');
        pvtz_dlpno = self.read_ccsdt(path + 'pvtz_DLPNO_CCSDt');

        for key in pvdz_ccsdt:

            if(key == 'atomic_charge'):
                obs_dic[key] = (np.array(pvdz_ccsdt[key]) + np.array(pvtz_dlpno[key]) - np.array(pvdz_dlpno[key])).tolist();
            elif(key == 'bond_order'):
                output = [];
                d1 = [u[:2] for u in pvdz_dlpno[key]];
                d2 = [u[:2] for u in pvtz_dlpno[key]];
                for bond in pvdz_ccsdt[key]:
                    if(bond[:2] in d1):
                        b1 = pvdz_dlpno[key][d1.index(bond[:2])][2];
                    else: 
                        b1 = 0;
                    if(bond[:2] in d2):
                        b2 = pvtz_dlpno[key][d2.index(bond[:2])][2];
                    else: 
                        b2 = 0;
                ...
                obs_dic[key] = pvdz_ccsdt[key] + pvtz_dlpno[key] - pvdz_dlpno[key];
...
Nl = [p for p in Nl if os.path.isdir(os.path.join(read_dir, p))]
